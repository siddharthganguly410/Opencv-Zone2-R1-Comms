...
def plot_astar(result, all_results, env_cfg, save_path="r2_astar_results.png"):
    ...
    fig = plt.figure(figsize=(26, 15), facecolor="#0f1117")
    fig.suptitle(
        "ABU Robocon 2026 – R2  |  A* Optimal Path Planning  |  Meihua Forest",
        fontsize=20, fontweight="bold", color="white", y=0.99,
    )

    # Layout: forest panels take 5/6 width each (×2), summary gets remaining third
    # Use a 1-row × 3-col grid with large width ratios for the two forests
    gs = GridSpec(1, 3, figure=fig, wspace=0.18,
                  width_ratios=[5, 5, 3],
                  left=0.06, right=0.99, top=0.88, bottom=0.04)

    def style_ax(ax, title):
        ax.set_facecolor(BG); ax.set_title(title, color=TEXT, fontsize=15, pad=10)
    ...
    def draw_forest(ax, highlight=None, title="Forest Layout"):
        # Expand coordinate space so cells appear larger within the axes
        PAD = 0.25
        ax.set_facecolor(BG)
        ax.set_xlim(-PAD, COLS + PAD)
        ax.set_ylim(-PAD - 0.5, ROWS + PAD)   # extra bottom room for footer text
        ax.set_aspect("equal"); ax.axis("off")
        ax.set_title(title, color=TEXT, fontsize=16, pad=12, fontweight="bold")
        ...
        for b in range(1, NUM_BLOCKS + 1):
            ...
            ec = ACCENT if b in hl else "#666"
            lw = 3.0    if b in hl else 1.2
            ax.add_patch(mpatches.FancyBboxPatch(
                (col + 0.06, y + 0.06), 0.88, 0.88, boxstyle="round,pad=0.07",
                facecolor=col_map, edgecolor=ec, linewidth=lw))
            ax.text(col+0.5, y+0.67, str(b), ha="center", va="center",
                    fontsize=16, fontweight="bold", color="white")
            sym = ("📜" if b in env_cfg["r2_real"] else "⚠" if b==env_cfg["fake"]
                   else "R1" if b in env_cfg["r1"] else "")
            if sym: ax.text(col+0.5, y+0.26, sym, ha="center", va="center",
                            fontsize=13, color="white")
        for row_i in range(ROWS):
            label = ("ENTRY" if row_i==0 else "EXIT" if row_i==ROWS-1 else "")
            if label:
                ax.text(-0.12, ROWS-1-row_i+0.5, label, ha="right", va="center",
                        fontsize=11, color=SUCCESS if label=="EXIT" else ACCENT,
    ...
    ], loc="lower left", fontsize=11, facecolor="#0f1117",
    ...
    if result and result.get("success"):
        ...
        for b, steps in step_map.items():
            ...
            ax1.text(col+0.5, y+0.5, "#" + ",".join(str(x) for x in steps[:3]),
                     ...
                     fontweight="bold",
                     bbox=dict(boxstyle="round,pad=0.2", fc=ACCENT, ec="none", alpha=0.9))
        prev = None
        for s in path:
            b = s["block"]
            if prev and prev != b:
        ...
        ax1.text(0.5, -0.08,
            ...
            f"|  Steps={len(path)}",
            transform=ax1.transAxes, ha="center", fontsize=12, color=ACCENT,
            fontweight="bold")

    # ── Panel 2: Solution summary ──
    ax5 = fig.add_subplot(gs[0, 2])
    ax5.set_facecolor(BG); ax5.axis("off")
    ax5.set_title("Solution Summary", color=TEXT, fontsize=16, pad=12, fontweight="bold")
    lines = [
    ...
    for i, (lbl, val, col) in enumerate(lines):
        y = 0.95 - i * 0.075
        ax5.text(0.04, y, lbl+":", transform=ax5.transAxes, fontsize=12, color=TEXT)
        ax5.text(0.52, y, val,    transform=ax5.transAxes, fontsize=12,
